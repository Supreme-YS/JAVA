...
public class LotteMartServiceTest3 {
	public static void main(String[] args) {
		...
		Product[] products = service.productsByCustomer(customers[0]);
		for (Product p : products) System.out.println(p.getDetails());
		...
		System.out.println("\n===== 3. 특정 고객을 검색한 결과입니다.=============");
		Customer cust = service.findCustomer(customers, 111);
		System.out.println(cust.customerInfo());
		
		System.out.println("\n===== 4. 다음은 동일한 동네에 거주하는 고객명단 입니다.=============");
		Customer[] custs = service.findCustomer(customers, "신사동");
		...
		int maxPrice = service.getMaxPrice(customers);
		System.out.println(maxPrice);
		
		System.out.println("\n===== 6. 고객님께서 구입한 상품중 1만원 이상 상품목록입니다.=============");
		Product[] morePriceProducts = service.getMorePriceProducts(customers, 10000);
		for (Product p : morePriceProducts) System.out.println(p.getDetails());
